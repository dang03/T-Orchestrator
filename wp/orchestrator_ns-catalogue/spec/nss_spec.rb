...
RSpec.describe OrchestratorNsCatalogue do
	...
	describe 'POST /network-services' do
		...
		context 'given a valid NS' do
			let(:response) { post '/network-services', {id: 1, name: 'teste'}.to_json, rack_env={'CONTENT_TYPE' => 'application/json'} }
	...
	describe 'GET /network-services/:id' do
		...
		context 'when the NS is not found' do
			let(:response_not_found) { get '/network-services/' + (ns._id.to_i + 1).to_s }
		...
		context 'when the NS is found' do
			let(:response_found) { get '/network-services/' + ns._id }
	...
	describe 'DELETE /network-services/:id' do
		...
		context 'when the NS is not found' do
			let(:response_not_found) { delete '/network-services/' + (ns._id.to_i + 1).to_s }
